...
noElementsAp = noElementsAp -1;
        ...
        %Bm(z) values on .h
i = 1;
noElementsBm =numel(Bm);
while (noElementsBm > 0)
fprintf(fileID,'#define Bm%d  %f\n',i,Bm(i));
i = i +1 ;
noElementsBm = noElementsBm -1;
end

        %Am(z) values on .h
i = 1;
noElementsAm =numel(Am);
while (noElementsAm > 0)
fprintf(fileID,'#define Am%d  %f\n',i,Am(i));
i = i +1 ;
noElementsAm = noElementsAm -1;
end
...
fileID = fopen('RSTParameters.h','w');
%R parameters
noElementsR = numel(R);
i = 1;
fprintf(fileID,'#ifndef RSTPARAMETERS_H\n');
fprintf(fileID,'#define RSTPARAMETERS_H\n');
while (noElementsR > 0)
fprintf(fileID,'#define R%d  %f\n',i,R(i));
i = i +1 ;
noElementsR = noElementsR -1;
end
...
fprintf(fileID, '{\n');
fprintf(fileID, '}\n');
fprintf(fileID, 'void Subsystem_terminate(void)\n');
fprintf(fileID, '{\n');
fprintf(fileID, '}\n');
fclose(fileID);
%end of writting subsystem.c function

%start writting Subsystem.h"
fileID = fopen('Subsystem.h','w');
fprintf(fileID, '#ifndef RTW_HEADER_Subsystem_h_\n' );
fprintf(fileID, '#define RTW_HEADER_Subsystem_h_\n' );
fprintf(fileID, '#ifndef Subsystem_COMMON_INCLUDES_\n');
fprintf(fileID, '# define Subsystem_COMMON_INCLUDES_\n');
fprintf(fileID,'#include "rtwtypes.h"\n' );
fprintf(fileID, '#endif\n' );
fprintf(fileID, '#ifndef rtmGetErrorStatus\n');
fprintf(fileID, '# define rtmGetErrorStatus(rtm)        ((rtm)->errorStatus)\n' );
fprintf(fileID, '#endif\n' );
fprintf(fileID, '#ifndef rtmSetErrorStatus\n');
fprintf(fileID, '# define rtmSetErrorStatus(rtm, val)   ((rtm)->errorStatus = (val))\n');
fprintf(fileID, '#endif\n');
fprintf(fileID, 'typedef struct tag_RTM RT_MODEL;\n');
fprintf(fileID, 'typedef struct {\n');
fprintf(fileID, 'real_T Tz_states[2];\n' );
fprintf(fileID, 'real_T Rz_states[2];\n');
fprintf(fileID, 'real_T uSz_states[2]; \n');
fprintf(fileID, '} DW;\n');
fprintf(fileID, 'typedef struct {\n');
fprintf(fileID, 'real_T In1;\n' );
fprintf(fileID, 'real_T In2; \n' );
fprintf(fileID, '} ExtU;\n');
fprintf(fileID, 'typedef struct {\n');
fprintf(fileID, ' real_T Out1; \n');
fprintf(fileID, '} ExtY;\n');
fprintf(fileID, 'struct tag_RTM {\n' );
fprintf(fileID, 'const char_T * volatile errorStatus;\n');
fprintf(fileID, '};\n');
fprintf(fileID, 'extern DW rtDW;\n');
fprintf(fileID, 'extern ExtU rtU;\n');
fprintf(fileID, 'extern ExtY rtY;\n' );
fprintf(fileID, 'extern void Subsystem_initialize(void);\n' );
fprintf(fileID, 'extern void Subsystem_step(void);\n' );
fprintf(fileID, 'extern void Subsystem_terminate(void);\n');
fprintf(fileID, 'extern RT_MODEL *const rtM;\n');
fprintf(fileID, '#endif  \n');
fclose(fileID);
%END Defining Subsystem.h

%file rtwtypes.h
fileID = fopen('rtwtypes.h','w');
fprintf(fileID, '#ifndef RTWTYPES_H\n' );
fprintf(fileID, '#define RTWTYPES_H\n' );
fprintf(fileID, '#if (!defined(__cplusplus))\n');
fprintf(fileID, '#  ifndef false\n');
fprintf(fileID,'#   define false                       (0U)\n' );
fprintf(fileID, '#  endif\n' );
fprintf(fileID, '#  ifndef true\n');
fprintf(fileID, '#  define true                        (1U)\n' );
fprintf(fileID, '#  endif\n' );
fprintf(fileID, '#endif\n');
fprintf(fileID, 'typedef signed char int8_T;\n');
fprintf(fileID, 'typedef unsigned char uint8_T;\n');
fprintf(fileID, 'typedef short int16_T;\n');
fprintf(fileID, 'typedef unsigned short uint16_T;\n');
fprintf(fileID, 'typedef int int32_T;\n' );
fprintf(fileID, 'typedef unsigned int uint32_T;\n');
fprintf(fileID, 'typedef float real32_T; \n');
fprintf(fileID, 'typedef double real64_T;\n');
fprintf(fileID, 'typedef double real_T;\n');
fprintf(fileID, 'typedef double time_T;\n' );
fprintf(fileID, 'typedef unsigned char boolean_T; \n' );
fprintf(fileID, 'typedef int int_T;\n');
fprintf(fileID, 'typedef unsigned int uint_T;\n');
fprintf(fileID, ' typedef unsigned long ulong_T; \n');
fprintf(fileID, 'typedef char char_T;\n');
fprintf(fileID, 'typedef unsigned char uchar_T;\n' );
fprintf(fileID, 'typedef char_T byte_T;\n');
fprintf(fileID, '#define MAX_int8_T                     ((int8_T)(127))\n');
fprintf(fileID, '#define MIN_int8_T                     ((int8_T)(-128))\n');
fprintf(fileID, '#define MAX_uint8_T                    ((uint8_T)(255U))\n');
fprintf(fileID, '#define MAX_int16_T                    ((int16_T)(32767))\n');
fprintf(fileID, '#define MIN_int16_T                    ((int16_T)(-32768)) \n');
fprintf(fileID, '#define MAX_uint16_T                   ((uint16_T)(65535U))\n');
fprintf(fileID, '#define MAX_int32_T                    ((int32_T)(2147483647))\n' );
fprintf(fileID, '#define MIN_int32_T                    ((int32_T)(-2147483647-1))\n');
fprintf(fileID, '#define MAX_uint32_T                   ((uint32_T)(0xFFFFFFFFU))\n');
fprintf(fileID, 'typedef void * pointer_T;\n');
fprintf(fileID, '#endif\n');
fclose(fileID);
%END writting rtwtypes.h
...
fprintf(fileID, '#include "Subsystem.h"\n');
fprintf(fileID, '#include "rtwtypes.h"\n');
fprintf(fileID,'void rt_OneStep(void);\n' );
fprintf(fileID, 'void rt_OneStep(void)\n' );
fprintf(fileID, '{\n');
fprintf(fileID, '  static boolean_T OverrunFlag = false;\n' );
fprintf(fileID, 'if (OverrunFlag) {\n' );
fprintf(fileID, 'rtmSetErrorStatus(rtM, "Overrun");\n');
fprintf(fileID, 'return;\n');
fprintf(fileID, '  }\n');
fprintf(fileID, '  OverrunFlag = true;\n');
fprintf(fileID, 'Subsystem_step();\n');
fprintf(fileID, 'OverrunFlag = false;\n' );
fprintf(fileID, '}\n');
fprintf(fileID, 'int_T main(int_T argc, const char *argv[]) \n');
fprintf(fileID, '{\n');
fprintf(fileID, '  (void)(argc);');
fprintf(fileID, '  (void)(argv);\n' );
fprintf(fileID, 'Subsystem_initialize();\n');
fprintf(fileID, 'printf("Warning: The simulation will run forever. ");\n');
fprintf(fileID, '  fflush((NULL));\n');
fprintf(fileID, 'while (rtmGetErrorStatus(rtM) == (NULL)) {\n');
fprintf(fileID, '  }\n');
fprintf(fileID, 'Subsystem_terminate();\n' );
